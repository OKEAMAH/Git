...
let list_recursively filename_filter directory =
  let rec list_recursively directory file_accumulator =
    ...
    |> Array.fold_left
         (fun file_accumulator entry ->
           let entry_path = Filename.concat directory entry in
           match Sys.is_directory entry_path with
           | true -> list_recursively entry_path file_accumulator
           | false ->
               if filename_filter entry then entry_path :: file_accumulator
               else file_accumulator
           | exception Sys_error _ -> file_accumulator)
         file_accumulator
  in
...
let list_non_recursively filename_filter directory =
  Sys.readdir directory |> Array.to_list |> List.filter filename_filter
...
let list_coverage_files files_on_command_line coverage_search_paths =
  ...
  let individual_files, coverage_search_paths =
    ...
    | [], [] ->
        ...
        in
        (files, paths)
    | _ -> (files_on_command_line, coverage_search_paths)
  in
  ...
  let all_coverage_files =
    ...
    |> List.flatten |> ( @ ) individual_files
  in

  (* Display feedback about where coverage files were found. *)
  (match (files_on_command_line, coverage_search_paths) with
  | [], [] | _, _ :: _ ->
      all_coverage_files |> List.map Filename.dirname
      |> List.sort_uniq String.compare
      |> List.map (fun directory -> directory ^ Filename.dir_sep)
      |> List.iter (Util.info "found *.coverage files in '%s'")
  | _ -> ()) ;

  if all_coverage_files = [] then Util.fatal "no *.coverage files found"
  else all_coverage_files
...
let is_source_file filename =
  List.exists
    (fun extension -> Filename.check_suffix filename extension)
    [".ml"; ".re"; ".mll"; ".mly"]
...
let filtered_expected_files expect do_not_expect =
  ...
  expected_files |> List.filter (fun path -> not (List.mem path excluded_files))
...
let get_relative_path file =
  if Filename.is_relative file then file
    ...
    try
      if
        String.sub file 0 cwd_end = cwd
        && String.sub file cwd_end sep_length = Filename.dir_sep
      then String.sub file sep_end (String.length file - sep_end)
      else file
...
let read ~coverage_file =
  ...
  try
    let magic_number_in_file =
      really_input_string
        channel
        (String.length Bisect_common.coverage_file_identifier)
    in
    ...
    result
  with
  | End_of_file -> Util.fatal "coverage file '%s' is truncated" coverage_file
  | Sys_error message ->
...
let load_coverage ~coverage_files ~coverage_paths ~expect ~do_not_expect =
  ...
  let present_files =
    Hashtbl.fold (fun filename _ acc -> filename :: acc) coverage []
  in
