(* Implementation of fast amortized Kate proofs (https://github.com/khovratovich/Kate/blob/master/Kate_amortized.pdf) *)

module Kate_amortized_impl = struct
  ...
  let commit p (srs1, _) =
    ...
    Plonk.Kzg.Kzg_impl.Commitment.commit_kate_amortized srs1 p
  ...
  module Domain = struct
    ...
    let inverse domain =
      ...
      Array.init n (fun i -> if i = 0 then Scalar.one else domain.(n - i))
  end
  ...
  let build_h_list_with_precomputed_srs a_list (domain2m, precomputed_srs) =
    let y = Option.get precomputed_srs in
  ...
  let build_h_list_complete a_list srs domain2m m =
    ...
    let v = Scalar.fft ~domain:domain2m ~points:a_list in
    let u = Array.map2 (fun yi vi -> G1.mul yi vi) y v in
    let res = G1.ifft ~domain:(Domain.inverse domain2m) ~points:u in
    Array.sub res 0 m
  ...
  *)
  let build_ct_list ~nb_proofs ~degree coefs (srs1, _srs2) domain2m =
    (* dump f₀ because we don’t need it for computation ; add zero at the end of list to maintain size *)
    let coefs = List.tl coefs @ [Scalar.zero] in
    let h_list =
      (* Computed following https://alinush.github.io/2020/03/19/multiplying-a-vector-by-a-toeplitz-matrix.html *)
      let padded_srs =
        List.rev_append srs1 (List.init degree (fun _ -> G1.zero))
      in
      ...
      let a_list =
        ...
        let rec fill_with_zero_and_fm m acc =
          if m = 0 then fm :: acc
          else fill_with_zero_and_fm (m - 1) (Scalar.zero :: acc)
        in
  ...
  (* precompute first part of Toeplitz trick, which doesn't depends on the polynomial’s coefficients *)
      ...
      if quotient = 0 then None
        ...
        let srsj =
          Array.init
            ((2 * quotient) + padding)
            (fun i ->
              if i < quotient then srs.(degree - j - ((i + 1) * l)) else G1.zero)
        in
        Some (build_srs_part_h_list srsj domain2m)
    in
  ...
  *)
  let multiple_multi_reveals_with_preprocessed_srs ~chunk_len ~chunk_count
    ...
    let l = 1 lsl chunk_len in
    (* Since we don’t need the first coefficient f₀, we remove it and add a zero as last coefficient to keep the size unchanged *)
    let coefs = Array.of_list (List.tl coefs @ [Scalar.zero]) in
    let compute_h_j j =
      let rest = (degree - j) mod l in
      let quotient = (degree - j) / l in
      if quotient = 0 then None
      else
        (* padding in case quotient is not a power of 2 to get proper fft in Toeplitz matrix part *)
        let padding = diff_next_power_of_two (2 * quotient) in
        let a_list =
          (* fm, 0, …, 0, f₁, f₂, …, fm-1 *)
          let a_array =
            Array.init
              ((2 * quotient) + padding)
              (fun i ->
                if i <= quotient + (padding / 2) then Scalar.zero
  ...
  let multiple_multi_reveals ~chunk_len ~chunk_count ~degree f srs =
    ...
    let preprocess = preprocess_multi_reveals ~chunk_len ~degree srs in
    ...
    let proof =
      multiple_multi_reveals_with_preprocessed_srs
        ...
        preprocess
    in
    Printf.printf "\nproofs : %f s" (Unix.gettimeofday () -. time) ;
    proof
  ...
  let verify cm_f (srs1, srs2l) domain (w, evaluations) proof =
    ...
    let sl_min_yl =
      G2.(add srs2l (negate (mul one (Scalar.pow w (Z.of_int l)))))
    in
    let diff_commits = G1.(add cm_h (negate cm_f)) in
    Pairing.pairing_check [(diff_commits, G2.one); (proof, sl_min_yl)]
end
...
module type Kate_amortized_sig = sig
  ...
  (* [build_ct_list ~nb_proofs:2ⁿ ~degree:m [f₀, f₁, …, fm-1] srs domain2m] returns multiple proofs for polynomial f₀ + f₁X + … on the 2ⁿ-th roots of unity
  *)
  val build_ct_list :
    nb_proofs:int ->
    degree:int ->
  ...
  (* [multiple_multi_reveals_with_preprocessed_srs ~chunk_len:r ~chunk_count:(n-r) ~degree:m [f₀, f₁, …, fm-1] precomputed] returns the 2ⁿ⁻ʳ proofs (each proof stands for for 2ʳ evaluations) for polynomial f₀ + f₁X + … as in part 3.2 *)
  ...
  (* same as multiple_multi_reveals_with_preprocessed_srs without preprocessing the SRS computations *)
  ...
  (* [verify cm_f srs domain (w, evaluations) proof] returns true iff for all i, f(w×domain.(i) = evaluations.(i)) *)
  val verify :
    commitment -> srs -> Domain.t -> Scalar.t * Scalar.t array -> proof -> bool
end

include (Kate_amortized_impl : Kate_amortized_sig)

module Reed_solomon = Reed_solomon
