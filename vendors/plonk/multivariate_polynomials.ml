...
module type MultiPoly_sig = sig
  ...
  module MonomialMap : Map.S with type key = Monomial.t
...
module MultiPoly
    (K : Ff_sig.PRIME)
    (UPoly : Polynomial.UNIVARIATE with type scalar = K.t) :
  ...
  module Monomial = struct
    ...
    let add_prefix prefix monomial =
      StringMap.Aggregation.prefix_map ~prefix monomial
