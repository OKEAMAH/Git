...
void reorg_fr_array_coefficients(blst_fr *coefficients, int n, int logn) {
  blst_fr tmp;
  for (int i = 0; i < n; i++) {
    int reverse_i = bitreverse_(i, logn);
    if (i < reverse_i) {
...
void fft_inplace_aux(blst_fr *coefficients, blst_fr *domain,
                     int log_domain_size, int inverse_domain) {

  int domain_size = 1 << log_domain_size;
  reorg_fr_array_coefficients(coefficients, domain_size, log_domain_size);
...
void fft_inplace(blst_fr *coefficients, blst_fr *domain, int log_domain_size) {
  fft_inplace_aux(coefficients, domain, log_domain_size, 0);
}

void ifft_inplace(blst_fr *coefficients, blst_fr *domain, int log_domain_size) {
  int domain_size = 1 << log_domain_size;
  u_int64_t n[4] = {domain_size, 0, 0, 0};
  blst_fr inverse_n;

  fft_inplace_aux(coefficients, domain, log_domain_size, 1);
...
// caml enforce that the first poly the biggest size
void add(blst_fr *res, blst_fr *poly_1, blst_fr *poly_2, const int size_1,
         const int size_2) {
  for (int i = 0; i < size_2; i++) {
...
void sub(blst_fr *res, blst_fr *poly_1, blst_fr *poly_2, const int size_1,
         const int size_2) {
  if (size_1 >= size_2) {
    for (int i = 0; i < size_2; i++) {
...
void mul(blst_fr *res, const blst_fr *poly_1, const blst_fr *poly_2,
...
bool is_zero(blst_fr *poly, const int size) {
  bool is_zero = 1;
  for (int i = 0; i < size; i++) {
...
void division_x_z(blst_fr *res, const blst_fr *poly, const int size,
                  blst_fr *z) {
...
void division_zs(blst_fr *res, const blst_fr *poly, const int size,
                 const int n) {
...
void truncate_inplace(blst_fr *poly, const int k, const int size) {
  u_int64_t zero[4] = {0}; 
  for (int i = k; i < size; i++) {
    blst_fr_from_uint64(poly + i, zero);
  };
}

void negate_truncated_inplace(blst_fr *poly, int k, int size) {
  for (int i = 0; i < k; i++) {
    blst_fr_cneg(poly + i, poly + i, 1);
  };
}
