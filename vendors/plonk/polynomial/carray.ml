...
module Stubs = struct
  ...
  external of_fr_array_truncated : fr array -> fr_array -> int -> int -> unit
    = "caml_of_fr_array_truncated_stubs"
    [@@noalloc]
...
module Carray : sig
  ...
  let to_array_truncated (p, n) k =
    let d = Array.init k (fun _ -> Fr.(copy zero)) in
    Stubs.of_fr_array_truncated d p n k ;
    d
