...
module Fr_generation = Plonk.Fr_generation.Make (Bls12_381.Fr)
module Poly = Polynomial.MakeUnivariate (Bls12_381.Fr)
module Poly_c = Plonk.Polynomial_c.Polynomial_impl

let p_of_c poly = Poly_c.to_sparse_coefficients poly |> Poly.of_coefficients

let test_build_domain () =
  let log = 12 in
  let root_of_unity = Fr_generation.root_of_unity log in
  let domain_c = Poly_c.Domain.create log root_of_unity in
  let expected_domain = Fr_generation.build_domain log in
  let domain = Poly_c.Domain.to_array domain_c in
...
let test_division_zs () =
  ...
  let poly_non_divisible =
    let rec generate () =
      let p = Poly_c.generate_random_polynomial 100 in
      if Poly_c.length p >= 2 * n then p else generate ()
    in
    generate ()
  in
  let poly_non_divisible_caml = p_of_c poly_non_divisible in
...
let test_vectors_fft () =
  ...
  List.iter
    (fun (points, expected_fft_results, root, n) ->
      ...
      let polynomial = Poly_c.of_coefficients points in
      let domain = Poly_c.Domain.create log root in
      let evaluations = Poly_c.evaluation_fft domain polynomial in
      assert (Array.for_all2 Fr.eq expected_fft_results evaluations))
    vectors
...
let test_ifft_random () =
  ...
  let domain = Poly_c.Domain.create log root in
  let fft_results = Poly_c.evaluation_fft domain p in
  let res = Poly_c.interpolation_fft domain fft_results in
  assert (Poly_c.equal p res)

let test_fft_interpolate () =
  let root = Fr_generation.root_of_unity 4 in
  let n = 16 in
  let log = Z.(log2up @@ of_int n) in
  let domain = Poly_c.Domain.create log root in
  let scalars = Array.init n (fun _ -> Fr.random ()) in
  let polynomial = Poly_c.interpolation_fft domain scalars in
  Array.iteri
    (fun i x ->
      assert (Fr.eq x (Poly_c.evaluate polynomial (Poly_c.Domain.get domain i))))
    scalars
...
let test_fft_evaluate () =
  ...
  let expected_result =
    Array.init n (fun i ->
        Poly_c.evaluate polynomial (Poly_c.Domain.get domain i))
  in
  let result = Poly_c.evaluation_fft domain polynomial in
  assert (Array.for_all2 Fr.eq result expected_result)

let test_domain_to_array () =
  let root = Fr_generation.root_of_unity 4 in
  let domain = Poly_c.Domain.create 4 root in
  let domain_caml = Poly_c.Domain.to_array domain in
