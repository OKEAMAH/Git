...
fn typecheck_instruction(
    ...
    // pop!() force-pops the stack (panics if nothing to pop)
    // pop!(T::Foo) force-pops the stack and unwraps T::Foo(x), retuning x
    // pop!(T::Bar, x, y, z) force-pops the stack, unwraps T::Bar(x, y, z) and
    // binds those to x, y, z in the surrounding scope.
    macro_rules! pop {
        ($($args:tt)*) => {
            irrefutable_match!(
