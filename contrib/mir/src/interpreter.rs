...
mod interpreter_tests {
    ...
    fn ticket() {
        ...
        let start_milligas = ctx.gas.milligas();
        assert_eq!(interpret(&vec![Ticket], &mut ctx, &mut stack), Ok(()));
    ...
    fn split_ticket() {
        ...
        let start_milligas = ctx.gas.milligas();
        assert_eq!(interpret(&vec![SplitTicket], &mut ctx, &mut stack), Ok(()));
    ...
    fn join_tickets() {
        ...
        let start_milligas = ctx.gas.milligas();
        assert_eq!(interpret(&vec![JoinTickets], &mut ctx, &mut stack), Ok(()));
        assert_eq!(
            start_milligas - ctx.gas.milligas(),
