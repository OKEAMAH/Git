...
impl ByteReprTrait for AddressHash {
    ...
    fn to_bytes(&self, out: &mut Vec<u8>) {
        use AddressHash::*;
        fn smart_contract(out: &mut Vec<u8>, tag: u8, hash: impl AsRef<Hash>) {
            out.push(tag);
            out.extend_from_slice(hash.as_ref());
            out.extend_from_slice(PADDING_SMART);
        }
        match self {
            Implicit(hash) => {
            ...
            Kt1(hash) => smart_contract(out, TAG_KT1, hash),
            Sr1(hash) => smart_contract(out, TAG_SR1, hash),
        }
    }
}
