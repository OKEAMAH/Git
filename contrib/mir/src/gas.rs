...
pub mod interpret_cost {
    ...
    pub fn and_num<T1: BigIntByteSize, T2: BigIntByteSize>(
        i1: &T1,
        i2: &T2,
    ) -> Result<u32, OutOfGas> {
    ...
    pub fn and_bytes(b1: &Vec<u8>, b2: &Vec<u8>) -> Result<u32, OutOfGas> {
        // fun size1 -> fun size2 -> (35. + (0.5 * (min size1 size2)))
        let sz = Checked::from(Ord::min(b1.len(), b2.len()));
        (35 + (sz >> 1)).as_gas_cost()
    }

    pub fn or_num<T1: BigIntByteSize, T2: BigIntByteSize>(
        i1: &T1,
        i2: &T2,
    ) -> Result<u32, OutOfGas> {
        let sz = Checked::from(Ord::min(i1.byte_size(), i2.byte_size()));
    ...
    pub fn or_bytes(b1: &Vec<u8>, b2: &Vec<u8>) -> Result<u32, OutOfGas> {
    ...
    pub fn xor_bytes(b1: &Vec<u8>, b2: &Vec<u8>) -> Result<u32, OutOfGas> {
        let sz = Checked::from(Ord::min(b1.len(), b2.len()));
        (40 + (sz >> 1)).as_gas_cost()
    }
