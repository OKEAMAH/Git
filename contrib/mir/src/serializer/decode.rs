...
/// If the number of annotations is small, an allocation-avoiding optimization
/// is used. This constant specifies the upper bound for the number of
/// annotations where it triggers.
const EXPECTED_MAX_ANNOTATIONS: usize = 5;
...
impl<'a> Micheline<'a> {
    ...
    pub fn decode_raw(
        ...
        if it.peek().is_some() {
            ...
            return Err(DecodeError::TrainlingBytes);
        }
        Ok(res)
    }
...
fn decode_seq_raw<'a>(
    arena: &'a Arena<Micheline<'a>>,
    bytes: &mut BytesIt<'a>,
) -> Result<SmallVec<[Micheline<'a>; EXPECTED_MAX_APP_ARGS]>, DecodeError> {
...
fn decode_seq<'a>(
    ...
    let buf = decode_seq_raw(arena, bytes)?;
    let res = Micheline::Seq(arena.alloc_extend(buf));
    Ok(res)
}

fn validate_ann(bytes: &[u8]) -> Result<Annotation, DecodeError> {
    ...
    Ok(Annotation::try_from(str).unwrap())
}
...
fn decode_app<'a>(
    ...
    let anns = if annotations {
        ...
        if str.is_empty() {
            ...
            let anns: SmallVec<[_; EXPECTED_MAX_ANNOTATIONS]> = str
                .split(|c| c == &0x20)
                .map(validate_ann)
                .collect::<Result<_, DecodeError>>()?;
            Annotations::from(anns)
        }
    } else {
        NO_ANNS
    };
...
mod test {
    ...
    mod value {
        ...
        #[test]
        fn errors() {
            check_err("0x030b00", DecodeError::TrainlingBytes);
