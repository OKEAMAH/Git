...
mod tests {
    use super::*;
    use crate::ast::{app, seq};

    fn parse(s: &str) -> Result<Micheline, ParseError<usize, Tok, ParserError>> {
        let parser = Box::leak(Box::new(Parser::new()));
        parser.parse(s)
    }

    fn parse_contract_script(s: &str) -> Result<Micheline, ParseError<usize, Tok, ParserError>> {
        let parser = Box::leak(Box::new(Parser::new()));
    ...
    fn pair_type() {
        ...
        assert_eq!(
            ...
            app!(PUSH[
              app!(pair[app!(pair[app!(int), app!(nat)]), app!(unit)]), ()
            ])
        );
    }
    ...
    fn or_type() {
        assert_eq!(
            ...
            app!(PUSH[app!(or[app!(int), app!(nat)]), ()])
        );
    ...
    fn contract_ty_push() {
        assert_eq!(
            ...
            app!(PUSH[app!(contract[app!(unit)]), ()])
        );
    }
