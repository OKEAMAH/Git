...
validate_kaitai_spec() {
    ...
    input_filename="01"
    tmp=$(mktemp -d)
    hex_dir="contrib/lib_kaitai_of_data_encoding/test/input/${encoding}"
    ...
    cat "$parser_path"
    echo
    for hex_file in "$hex_dir"/*; do
        parse_valid_hex_input "$tmp" "$hex_file" "$encoding" "$parser_path"
