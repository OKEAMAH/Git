...
# and Adaptive inflation activation
#
# Prerequisite:
# - Patched branch with
#   + "baken <nb blocks> for" command
#   + adaptive-inflation-vote toggle for bake for command
...
function build_yes-wallet(){
    ...
    FORCE=""
    if test -f $yes_wallet/*_highwatermarks; then
        ask "   yes wallet contains highwatermarks, clean them ?" || return
        clean_yes-wallet
    fi
    if test -f $yes_wallet/public_keys; then
        ask "   yes wallet already exists, force overwrite ?" || return
        FORCE="--force"
    fi
    eval $(opam env)
    dune exec devtools/yes_wallet/yes_wallet.exe -- create from context $data_dir in $yes_wallet --active-bakers-only --staking-share 75 --network  $network $FORCE
}
...
function bake(){
    ...
    [[  "$1" -gt 0 ]] || fail "number of blocks to bake not specified"
    echo "    $tezos_dir/octez-client -d $yes_wallet baken $1 for --minimal-timestamp --ignore-node-mempool  >> $client_log  2>&1"
    $tezos_dir/octez-client -d $yes_wallet bake for --count $1 --minimal-timestamp --ignore-node-mempool >> $client_log  2>&1 || \
        fail "Failed to bake $1 blocks (no vote)"
}

function bake_vote_on(){
    # bake the number of provided blocks
    echo "  Baking: $1 blocks (adaptive-inflation voting on)"
    [[  "$1" -gt 0 ]] || fail "number of blocks to bake not specified"
    echo "    $tezos_dir/octez-client -d $yes_wallet baken $1 for --minimal-timestamp --adaptive-inflation-vote on --ignore-node-mempool >> $client_log  2>&1 "
    $tezos_dir/octez-client -d $yes_wallet bake for --count $1 --minimal-timestamp --adaptive-inflation-vote on --ignore-node-mempool >> $client_log  2>&1 || \
        fail "Failed to bake $1 blocks (adaptive-voting on) "
}
...
function check_on_proto_alpha(){
    protocol=$(rpc /chains/main/blocks/head/header | jq .protocol)
    [[ $protocol =~ "ProtoALpha" ]] || fail "/!\ Context is on $protocol,not proto_alpha. exiting."
...
function extract_tarball(){
    ...
    extracted_data_dir=$data_dir/${network}-${history_mode}
    if test -d "$extracted_data_dir/store"; then
        echo "   Moving extracted data dir content into $data_dir"
        mv $extracted_data_dir/* $data_dir
    else
        fail "Invalid data_dir extracted in $data_dir"
    fi
}
...
## Adaptive inflation specifics
function compute_AI_activation_block(){
    echo "Computing AI activation block:"
    activation_cycle=$(rpc /chains/main/blocks/head/context/adaptive_inflation_launch_cycle)
    ...
    echo "   adaptive inflation will activate in $cycle_to_bake cycles at cycle $activation_cycle, current cycle is $current_cycle, baking $blocks_to_bake"
}

## bake until the vote for AI passes
function wait_adaptive_inflation_activation(){
    check_on_proto_alpha
    echo "Baking until adaptive inflation activation:"
    while [[ $(rpc /chains/main/blocks/head/context/adaptive_inflation_launch_cycle) == "null" ]]; do
        #baking 1000 times before giving feedback
        echo -n "   $(date) - no known date for AI activation, baking 1000 blocks - "
        bake_vote_on 1000
        date
        rpc /chains/main/blocks/head/helpers/current_level | jq -c;
    done
    compute_AI_activation_block
    bake_vote_on $blocks_to_bake
}
...
#  - bake until adaptive inflation activation
#
######################################################
function full_scenario(){
    ...
    if compute_UAU_parameters; then 
        ...
        echo "  Context is no more on $starting_proto, trying to activate adaptive inflation"
    fi
    wait_adaptive_inflation_activation
    node_stop
