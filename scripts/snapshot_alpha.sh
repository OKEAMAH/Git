...
mkdir /tmp/tezos_octez_snapshot
git archive HEAD src/proto_alpha/ | tar -x -C /tmp/tezos_octez_snapshot
# remove the README because it is specific to Alpha
rm /tmp/tezos_octez_snapshot/src/proto_alpha/README.md
mv /tmp/tezos_octez_snapshot/src/proto_alpha src/proto_${version}
rm -rf /tmp/tezos_octez_snapshot
...
sed -i.old.old -e 's/let version_value = "alpha_current"/let version_value = "'${current}'"/' \
    protocols/proto_${version}/constants_repr.ml\
    protocols/proto_${version}/raw_context.ml\
    src/proto_${version}/lib_client/proxy.ml

long_hash=$(./octez-protocol-compiler -hash-only protocols/proto_${version})
...
sed -e s/_alpha:/_${version}_${label}:/g \
    -e s,src/proto_alpha,src/proto_${version}_${short_hash},g \
    -e s,protocols/proto_alpha,src/protocols_${version}_${short_hash},g \
...
printf \\n$long_hash >> ../../src/lib_protocol_compiler/final_protocol_versions
