...
usage="Usage:
$ ./scripts/user_activate_update.sh protocols/proto_<version_number|alpha>* <level>
...
then
    ...
    pred_full_hash=$(jq -r .hash < protocols/proto_${pred}_*/lib_protocol/TEZOS_PROTOCOL)
    pred_short_hash=$(echo $pred_full_hash | head -c 8)

    full_hash=$(jq -r .hash < $1/lib_protocol/TEZOS_PROTOCOL)
else
    pred_version_dir=$(find src -regex "protocols/proto_[0-9][0-9][0-9]_[^/]*" -printf '%P\n' | sort -r | head -1)
    ...
    full_hash=$(jq -r .hash < protocols/proto_alpha/lib_protocol/TEZOS_PROTOCOL)
fi
level=$2
