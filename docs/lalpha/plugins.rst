...
:src:`src/proto_015_PtBsEUgi/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_lalpha`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_lalpha`).

.. _prevalidator_filters_lalpha:
...
on the :ref:`validation pass<validation_passes_lalpha>`.Some extra information (like the fees, or the gas/fees
