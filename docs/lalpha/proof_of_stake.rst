...
The PRNG's :ref:`seeds <random_seed_lalpha>` are obtained from random
...
:ref:`protocol constant <protocol_constants_lalpha>`.
...
PRNG having as seed the :ref:`random seed<random_seed_lalpha>` for
cycle ``n``.
...
snapshot<snapshots_lalpha>` for the cycle to which the level belongs.
...
:ref:`random seed<random_seed_lalpha>` for the cycle to which the
level belongs, and the slot.


.. _protocol_constants_lalpha:
...
In particular, the protocol constants related to the proof-of-stake mechanism are detailed :ref:`below <ps_constants_lalpha>`.
Other important subsystems of the protocol have their own list of protocol constants, such as the :ref:`consensus-related parameters <cs_constants_lalpha>` and the :ref:`randomness generation parameters <rg_constants_lalpha>`.

.. _ps_constants_lalpha:
