...
particular constraints web3 developers are very familiar with. Indeed,
the semantics of a smart rollup needs to be fully deterministic. As a
consequence,

#. Some instructions and types of the WASM language are forbidden,
   specifically the one related to floating-point arithmetic.
...
the smart rollups is that WASM has gradually become an ubiquitous
...
official target to compile ``.wasm`` binary file that are valid WASM
kernel. This means that, for this particular example, one can build a
WASM kernel while enjoying the strengths and convenience of the Rust
language and the Rust ecosystem.
...
#. A persistent state, that is preserved across ``kernel_next`` calls.
   ...
   populated with the inputs coming from the layer-1, the *output*
...
block. It is important to keep in mind that all the smart rollup
...
purpose from the one it does not need to process.
...
``write_output``
  Writes a in-memory buffer to the outbox of the smart rollup. If the
...
``store_get_nth_key``
  ...
  stable wrt. key additions and removals. Returns the number of bytes
  loaded in memory, which can be :math:`0` if the nth child is a
  value.
...
------- -------------------------------------------------------------------------------------------------------
  ...
  -4     The host functions tried to read or write an invalid section of the value stored under a given key
  -5     Cannot write to a value beyond the 2GB size limit
...
Note that this document is not a tutorial about Rust, and familiarity
with the language and its ecosystem (*e.g.*, how Rust crates are
...
eventually be consumed by a Kernel WASM crate, this line should be
modified to
...
.. code:: javascript
  ...
  [ { "payload" : <Micheline payload for the contract>,
      "sender" : <Contract hash of and originated contract for the rollup, optional>,
...
Note that the `sender` and `source` fields are optional and will be given
default values by the REPL. If no input file is given it will be assumed empty.
If no rollup address is given, it will use a default address.

``octez-wasm-repl`` is a REPL, as such it waits for user inputs to continue its
execution. Its initial state is exactly the same as right after its origination.
Its current state can be inspected via the command ``show current state``:
...
At start, internally the kernel is in snapshot mode, and waiting for inputs. It
needs some inputs to continue its execution. The command ``load inputs for level
N`` will load the inputs given with the option `--input` at the given level,
putting `Start_of_level` (and `End_of_level`) before (resp. after) these inputs.

.. code::

  $ load inputs for level 0
  Loaded N inputs at level 0

  $ show current state
  Status: Evaluating (X remaining)
  Internal state: Decode

At this point, the internal input buffer can be inspected with the commands
``show input N`` and ``show inbox``.
...
.. code::
  ...
  $ step kernel_next
  Evaluation took T ticks
  Status: Evaluating (X remaining)
  Internal state: Snapshot


As we can see, the status is `Evaluating (X remaining)`: the kernel will do at
least ``X * kernel_next`` before waiting for new inputs. The memory of the
interpreter is flushed between two `kernel_next` (at the `Snapshot` internal
state), however the ``durable storage`` can be used as a persistent memory.
Let's assume this kernel wrote data at key `/store/key`:
...
the PVM before its snapshot internal state, with ``step until result``, and
...
.. code::
  ...
  $ step kernel_next
  Evaluation took T ticks
  Status: Evaluating (<X-1> remaining)
  Internal state: Snapshot

Once again, note that values from the memory are outputed as is, since the
...
It is also possible to show the outbox (``show outbox``) or any output message
(``show output N``).
