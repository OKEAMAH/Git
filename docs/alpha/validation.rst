...
to the ``Full construction`` mode, and the difference boil down to the
...
- ``Outdated``: the operation is *too old* to be included in a
  ...
  consensus operations. Its semantics is close to ``Branch``, except
  that there might be still some value in the information provided by
  the operation. An example is the case of an endorsement which was
  received *too late*, but that could still be used to form a
  consensus quorum.
...
operation in the batch, by their inclusion order -- the head of the
batch being the first manager operation being applied.
