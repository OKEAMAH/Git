...
The PRNG's :ref:`seeds <random_seed_dryrun>` are obtained from random
...
:ref:`protocol constant <protocol_constants_dryrun>`.
...
PRNG having as seed the :ref:`random seed<random_seed_dryrun>` for
cycle ``n``.
...
snapshot<snapshots_dryrun>` for the cycle to which the level belongs.
...
:ref:`random seed<random_seed_dryrun>` for the cycle to which the
level belongs, and the slot.


.. _protocol_constants_dryrun:
...
In particular, the protocol constants related to the proof-of-stake mechanism are detailed :ref:`below <ps_constants_dryrun>`.
Other important subsystems of the protocol have their own list of protocol constants, such as the :ref:`consensus-related parameters <cs_constants_dryrun>` and the :ref:`randomness generation parameters <rg_constants_dryrun>`.

.. _ps_constants_dryrun:
