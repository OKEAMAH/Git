...
:src:`src/proto_015_PtTyfgNF/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_dryrun`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_dryrun`).

.. _prevalidator_filters_dryrun:
...
on the :ref:`validation pass<validation_passes_dryrun>`.Some extra information (like the fees, or the gas/fees
