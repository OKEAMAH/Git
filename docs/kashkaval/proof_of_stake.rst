...
The PRNG's :ref:`seeds <random_seed_kashkaval>` are obtained from random
...
:ref:`protocol constant <protocol_constants_kashkaval>`.
...
PRNG having as seed the :ref:`random seed<random_seed_kashkaval>` for
cycle ``n``.
...
snapshot<snapshots_kashkaval>` for the cycle to which the level belongs.
...
:ref:`random seed<random_seed_kashkaval>` for the cycle to which the
level belongs, and the slot.


.. _protocol_constants_kashkaval:
