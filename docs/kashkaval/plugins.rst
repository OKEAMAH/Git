...
:src:`src/proto_014_PsrBnRgf/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_kashkaval`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_kashkaval`).

.. _prevalidator_filters_kashkaval:
...
on the :ref:`validation pass<validation_passes_kashkaval>`.Some extra information (like the fees, or the gas/fees
