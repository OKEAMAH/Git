...
module Gitlab = struct
  ...
  let project_pipelines ?source ?ref_ ?sha ?status =
    make
  ...
  let project_job_artifacts ~job_id =
...
let fetch_pipeline_records_from_job pipeline ~job_name ~artifact_folder =
  ...
  | Some job ->
      ...
      let job_id = JSON.(job |-> "id" |> as_int) in
      let* () =
        Gitlab.(get_output ~output_path @@ project_job_artifacts ~job_id ())
      in
  ...
  let rec aux = function
    ...
    | commit :: commit_parent :: commits ->
        if is_merge_commit commit then (
          ...
          let* pipelines =
            Gitlab.(
              project_pipelines ~sha:(commit_hash commit_parent) () |> get)
          in
...
let get_last_default_branch_push_pipeline () =
  ...
  let* pipelines =
    Gitlab.(
      project_pipelines ~status:"success" ~source:"push" ~ref_:default_branch ()
      |> get)
  in
