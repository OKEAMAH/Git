...
(* Main entrypoint of CI-in-OCaml.
   ...
   We here register the set of pipelines, stages and images used to
...
let () =
  ...
  Pipeline.(
    ...
    register
      "test_non_release_tag"
      If.(
        not_on_tezos_namespace && push && has_tag_not_match any_release_tag_re) ;
    register
      "scheduled_extended_test"
...
(* Register images.
   ...
   For documentation onf the [runtime_X_dependencies] and the
...
module Images = struct
  ...
  let alpine = Image.register ~name:"alpine" ~image_path:"alpine:3.17"
end
...
let trigger =
  job
    ...
    ~rules:
      [
        job_rule
          ~if_:If.(Rules.merge_request && Rules.assigned_to_marge_bot)
          ~when_:Manual
          ();
