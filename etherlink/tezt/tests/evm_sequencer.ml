...
let test_resilient_to_rollup_node_disconnect =
  ...
  (* The objective of this test is to show that the sequencer can deal with
     ...
     wait for its advance on the rollup node is more than [max_blueprints_lag]
  ...
  let max_blueprints_catchup = max_blueprints_lag in
  ...
  (* Produce one block to update [latest_level_injected] *)
  let* _ = Rpc.produce_block evm_node in
  let* () = Evm_node.wait_for_blueprint_applied evm_node ~timeout:5. 21 in
  ...
  (* Finish the cooldown period to let the sequencer sends the rest of the
     blueprints. *)
  let* _ =
    repeat catchup_cooldown (fun () ->
        let* _ = next_rollup_node_level ~sc_rollup_node ~node ~client in
        unit)
  in
