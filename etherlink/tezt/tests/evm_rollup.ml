...
*)
let check_tx_gas_for_flat_fee ~flat_fee ~expected_execution_gas ~gas_price ~gas_used ~base_fee_per_gas =
  (* execution gas fee *)
  let expected_gas_used = Z.of_int expected_execution_gas in
  let expected_base_fee_per_gas = Z.of_int32 base_fee_per_gas in
  let execution_gas_fee = Z.mul expected_gas_used expected_base_fee_per_gas in
  Printf.printf "egf: %s\n" (Z.to_string execution_gas_fee);
  (* total fee 'in gas' *)
  let expected_total_fee = Z.add (Wei.of_wei_z flat_fee) execution_gas_fee in
  let expected_gas_used = Z.cdiv expected_total_fee (Z.of_int32 gas_price) |> Z.to_int64 in
  Check.((expected_gas_used = Int64.of_int32 gas_used) int64)
     ~error_msg: "unexpected gas in receipt (expected %L, actual %R)"
...
let transfer ?data ~flat_fee ~expected_execution_gas ~evm_setup () =
  ...
  Printf.printf "base_fee_per_gas: %s\n" (Int32.to_string base_fee_per_gas);
  let sender, receiver =
  ...
  (* if Option.is_none data then *)
    check_tx_gas_for_flat_fee ~flat_fee ~expected_execution_gas ~gas_used ~gas_price ~base_fee_per_gas;
  unit

let test_l2_transfer =
  let flat_fee = Wei.of_eth_string "0.000123" in
  let expected_execution_gas = 21000 in
  let test_f ~protocol:_ ~evm_setup = transfer ~evm_setup ~flat_fee ~expected_execution_gas () in
...
let test_chunked_transaction =
  ...
  let test_f ~protocol:_ ~evm_setup =
    transfer ~data:("0x" ^ String.make 12_000 'a') ~flat_fee ~evm_setup ~expected_execution_gas ()
  in
