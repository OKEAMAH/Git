...
let test_reboot_out_of_ticks =
  ...
  in
  let* _requests, _receipt, _hashes =
    send_n_transactions ~sc_rollup_node ~node ~client ~evm_node first_block
  in
  ...
  let* requests, receipt, _hashes =
    send_n_transactions ~sc_rollup_node ~node ~client ~evm_node second_block
  in
  ...
  in
  let block_with_out_of_ticks =
    block_with_out_of_ticks |> Evm_node.extract_result |> Block.of_json
  in
