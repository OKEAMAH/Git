...
/// This warning is disabled as ExitReason is defined by an external crate, as
/// such we obviously cannot implement the `From` trait on it.
#[allow(clippy::from_over_into)]
impl Into<ExitReason> for ExtendedExitReason {
    fn into(self) -> ExitReason {
        match self {
            Self::Exit(e) => e,
            // If an OutOfTicks cannot be recovered from, we consider it as an OutOfGas
            // error.
            Self::OutOfTicks => ExitReason::Error(ExitError::OutOfGas),
        }
    }
}
    ...
    fn end_initial_transaction(
        ...
        match execution_result {
            ...
            Err(EthereumError::OutOfTicks) => {
                ...
                self.rollback_initial_transaction(
                    None,
                    // The outcome built by the rollback is discarded, as such we can simpl
