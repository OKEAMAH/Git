...
use crate::accessed_storage::AccessedStorage;
...
struct TransactionLayerData<'config> {
    ...
    pub withdrawals: Vec<Withdrawal>,
    /// Keep track of accessed locations in durable storage.
    /// See EIP-2929 and YP section 6.1
    pub accessed_storage_keys: AccessedStorage,
}
...
impl<'config> TransactionLayerData<'config> {
    ...
    pub fn new(
        ...
        accessed_storage_keys: AccessedStorage,
    ) -> Self {
...
impl<'a, Host: Runtime> EvmHandler<'a, Host> {
    ...
    fn mark_storage_as_hot(
        ...
        layer.accessed_storage_keys.insert(address, index);
    ...
    fn is_storage_hot(&self, address: H160, index: H256) -> Result<bool, ExitError> {
        ...
        Ok(layer.accessed_storage_keys.contains(address, index))
    }
...
impl<'a, Host: Runtime> Handler for EvmHandler<'a, Host> {
    ...
    fn is_cold(&mut self, address: H160, index: Option<H256>) -> Result<bool, ExitError> {
        ...
        match index {
            Some(index) => self.is_storage_hot(address, index).map(|x| !x),
            None => Ok(false), // We should check if contract code is cold. STUB
        }
    }
        ...
        if let Some(cost) = evm::gasometer::static_opcode_cost(opcode) {
            ...
            match opcode {
                Opcode::SLOAD | Opcode::SSTORE => {
                    self.mark_storage_as_hot(context.address, stack.peek(0)?)
                }
                _ => Ok(( /* do nothing */ )),
            }
        }
