...
use storage::{
    ...
    store_base_fee_per_gas, store_chain_id, store_flat_fee, store_kernel_version,
...
fn retrieve_block_fees<Host: Runtime>(host: &mut Host) -> Result<BlockFees, Error> {
    ...
    let flat_fee = match read_flat_fee(host) {
        ...
        Err(_) => {
            ...
            log!(host, Debug, "default flat fee: {FLAT_FEE}");
            flat_fee
        }
    };

    let block_fees = BlockFees::new(base_fee_per_gas, flat_fee);
    log!(host, Debug, "loaded block fees {block_fees:?}");
