...
use evm_execution::{ArithmeticErrorKind, EthereumError};
...
pub fn tx_execution_gas_limit(
    ...
    tx.gas_limit_with_fees().checked_sub(gas_for_fees).ok_or(
        EthereumError::ArithmeticError(ArithmeticErrorKind::FeeUnderflow),
    )
}
...
const fn fee_overflow() -> EthereumError {
    EthereumError::ArithmeticError(ArithmeticErrorKind::FeeOverflow)
}

fn gas_as_u64(gas_for_fees: U256) -> Result<u64, EthereumError> {
    if gas_for_fees > U256::from(u64::MAX) {
        return Err(fee_overflow());
    }

    Ok(gas_for_fees.as_u64())
}
