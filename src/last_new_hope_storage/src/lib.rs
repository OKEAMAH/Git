...
pub fn write_diff_file(diff : &BTreeMap<u64, [u8 ; PAGE_SIZE]>)
{
    ...
    let index_file = OpenOptions::new().write(true).open(format!("./diffs/{diff_index}_index")).unwrap() ;
    let _ = index_file.set_len((cardinal as u64) * 8) ;
    let value_file = OpenOptions::new().write(true).open(format!("./diffs/{diff_index}")).unwrap() ;
...
pub fn update_storage(diff : &BTreeMap<u64, [u8 ; PAGE_SIZE]>) {
    let storage = OpenOptions::new().write(true).open(format!("./storage")).unwrap() ;
    diff.iter().for_each(|(k, v)| {
...
pub fn to_backward_diff(diff : &mut BTreeMap<u64, [u8 ; PAGE_SIZE]>) {
    let storage = OpenOptions::new().read(true).open(format!("./storage")).unwrap() ;
...
mod tests {
    ...
    fn it_works() {
        ...
        vec![(1_u64,[2_u8 ; PAGE_SIZE]), (3_u64,[4_u8 ; PAGE_SIZE])].into_iter().collect() ;
        //  BTreeMap::new();
        println!("{:?}", diff) ;
        nuke() ;
        init() ;
