...
use evm_execution::account_storage::{account_path, init_account_storage};
...
pub fn evm_kernel_run<Host: Runtime>(host: &mut Host) {
    ...
    if let Ok(b) = faucet.balance(host) {
        if b == primitive_types::U256::zero() {
            match faucet.balance_add(host, primitive_types::U256::max_value()) {
                Ok(_) => debug_msg!(host, "Faucet filled up"),
                Err(_) => debug_msg!(host, "Couldn't fill up faucet"),
            }
        } else {
            debug_msg!(host, "faucet already filled up")
        }
    } else {
        debug_msg!(host, "couldn't get faucet balance")
    }
