...
fn apply_transaction<Host: Runtime>(
    ...
    if !transaction.tx.clone().verify_signature() || sender_nonce != transaction.tx.nonce
    {
        Ok(make_receipt(
            block,
            transaction,
...
mod tests {
    ...
    use crate::genesis;
    use crate::inbox::Transaction;
    use crate::storage::read_transaction_receipts_status;
    ...
    fn test_invalid_transactions_receipt_status() {
        ...
        match read_transaction_receipts_status(&mut host, &tx_hash) {
