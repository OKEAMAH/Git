...
impl InputResult {
    ...
    fn parse_new_chunked_transaction(bytes: &[u8]) -> Self {
        ...
        if remaining.is_empty() {
            Self::Input(Input::NewChunkedTransaction { tx_hash, len })
        } else {
            Self::Unparsable
        }
    }
...
fn handle_transaction_chunk<Host: Runtime>(
    ...
    if i != len - 1 && data.len() < maximum_size {
        // Removes the whole chunked transaction?"
        return Ok(None);
    };
...
pub fn read_inbox<Host: Runtime>(
    ...
    loop {
        match read_input(host, smart_rollup_address)? {
            ...
            InputResult::Input(Input::NewChunkedTransaction { tx_hash, len }) => {
...
mod tests {
    ...
    fn input_to_bytes(smart_rollup_address: [u8; 20], input: Input) -> Vec<u8> {
        ...
        match input {
            Input::SimpleTransaction(tx) => {
                ...
                let mut tx_bytes = RawTransaction::to_bytes(&tx.tx).unwrap();
                buffer.append(&mut tx_bytes)
            }
            Input::NewChunkedTransaction { tx_hash, len } => {
                // New chunked transaction tag
                buffer.push(1);
                buffer.extend_from_slice(&tx_hash);
                buffer.extend_from_slice(&u16::to_le_bytes(len))
            }
    ...
    fn make_chunked_transactions(tx_hash: TransactionHash, data: Vec<u8>) -> Vec<Input> {
        let size_per_chunk = 4095 - 20 - 1 - 2 - 32;
        let mut chunks: Vec<Input> = data
            .chunks(size_per_chunk)
            .enumerate()
    ...
    fn parse_valid_simple_transaction() {
        ...
        let tx =
	    RawTransaction::decode_from_rlp(&hex::decode("f86d80843b9aca00825208940b52d4d3be5d18a7ab5e4476a2f5382bbf2b38d888016345785d8a000080820a95a0d9ef1298c18c88604e3f08e14907a17dfa81b1dc6b37948abe189d8db5cb8a43a06fc7040a71d71d3cb74bd05ead7046b10668ad255da60391c017eea31555f156").unwrap()).unwrap();
    ...
    fn parse_valid_chunked_transaction() {
        ...
        let tx = RawTransaction::decode_from_rlp(&data).unwrap();
