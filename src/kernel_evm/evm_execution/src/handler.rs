...
struct TransactionLayerData<'config> {
    ...
    pub withdrawals: Vec<Withdrawal>,
    /// Number of outbox messages up until this layer of transaction
    /// started. To make sure we don't produce too many outbox messages.
    pub outbox_counter: usize,
}
...
impl<'a, Host: Runtime> EvmHandler<'a, Host> {
    ...
    /// Current number of outbox messages. This includes _both_ those that were
    /// already produced after last level increment, but before current transaction,
    /// and also withdrawals produced in the current transaction.
    pub fn outbox_count(&self) -> Result<usize, EthereumError> {
        self.transaction_data
            .last()
            .map(|layer| Ok(layer.withdrawals.len() + layer.outbox_counter))
            .unwrap_or(Ok(0))
        /*
           // code below to be used once !10043 has been merged. That fixes the
           // precompiles module so that we test with a transaction in progress.
           // Code below will replace current function body of this function.

        let Some(layer) = self.transaction_data.last() else {
            return Err(EthereumError::InconsistentTransactionStack(0, false, false));
        };

        Ok(layer.withdrawals.len() + layer.outbox_counter)
        */
    }
    ...
    pub fn call_contract(
        ...
        self.begin_initial_transaction(is_static, gas_limit, outbox_counter)?;
    ...
    pub fn create_contract(
        ...
        self.begin_initial_transaction(false, gas_limit, outbox_counter)?;
    ...
    fn begin_inter_transaction(
        ...
        let Some(outbox_counter) = self.transaction_data.last().map(|l| l.outbox_counter + l.withdrawals.len()) else {
            return Err(EthereumError::InconsistentTransactionStack(0, false, true));
        };
...
mod test {
    ...
    fn origin_instruction_returns_origin_address() {
        ...
        handler.begin_initial_transaction(false, None, 0).unwrap();
