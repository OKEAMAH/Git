...
let rec encode ns =
  ...
  let+ ns = Lazy_vector.Immutable.LwtInt32Vector.to_list ns in
...
let encode_unsafe ns =
  let ns =
    List.map snd (Lazy_vector.Immutable.LwtInt32Vector.loaded_bindings ns)
  in
  encode_list ns
...
let rec decode s =
  Lazy_vector.Immutable.LwtInt32Vector.of_list
