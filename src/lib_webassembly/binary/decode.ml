...
module CBV = Immutable_chunked_byte_vector
...
type stream = {name : string; bytes : CBV.t; mutable pos : int}

let make_stream ~name ~bytes = {name; bytes; pos = 0}

let empty_block = Ast.Block_label 0l

let len64 s = CBV.length s.bytes
...
let read s = CBV.load_byte s.bytes (Int64.of_int s.pos)
