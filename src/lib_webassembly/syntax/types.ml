...
type result_type = value_type Lazy_vector.Immutable.LwtInt32Vector.t
...
let func_type_empty (FuncType (ins, out)) =
  Lazy_vector.Immutable.LwtInt32Vector.num_elements ins = 0l
  && Lazy_vector.Immutable.LwtInt32Vector.num_elements out = 0l
...
let func_types_equal (FuncType (ins, out)) (FuncType (ins', out')) =
  ...
  let ins_len = Lazy_vector.Immutable.LwtInt32Vector.num_elements ins in
  let out_len = Lazy_vector.Immutable.LwtInt32Vector.num_elements out in
  if
    ins_len <> Lazy_vector.Immutable.LwtInt32Vector.num_elements ins'
    || out_len <> Lazy_vector.Immutable.LwtInt32Vector.num_elements out'
  then Lwt.return_false
  else
    let rec for_all acc i len vec vec' =
      if i >= len || not acc then Lwt.return acc
      else
        let* t = Lazy_vector.Immutable.LwtInt32Vector.get i vec in
        let* t' = Lazy_vector.Immutable.LwtInt32Vector.get i vec' in
...
let string_of_result_type ts =
  (* TODO: https://gitlab.com/tezos/tezos/-/issues/3378
     ...
     testsuite. *)
  let ts =
    List.map snd (Lazy_vector.Immutable.LwtInt32Vector.loaded_bindings ts)
  in
