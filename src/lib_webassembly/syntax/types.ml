...
type result_type = value_type Vector.t
...
let func_type_empty (FuncType (ins, out)) =
  Vector.num_elements ins = 0l && Vector.num_elements out = 0l
...
let func_types_equal (FuncType (ins, out)) (FuncType (ins', out')) =
  ...
  let ins_len = Vector.num_elements ins in
  let out_len = Vector.num_elements out in
  if ins_len <> Vector.num_elements ins' || out_len <> Vector.num_elements out'
  then Lwt.return_false
  else
    let rec for_all acc i len vec vec' =
      if i >= len || not acc then Lwt.return acc
      else
        let* t = Vector.get i vec in
        let* t' = Vector.get i vec' in
...
let string_of_result_type ts =
  (* TODO: https://gitlab.com/tezos/tezos/-/issues/3378
     ...
     testsuite. *)
  let ts = Vector.Unsafe_for_tick.to_list ts in
