...
let invoke_step ~init ?(durable = Durable_storage.empty)
  ...
  | Inv_stop {ticks; _} when ticks = Z.zero ->
      raise (Evaluation_step_error (Invoke_step "Inv_stop cannot reduce"))
  | Inv_stop stop ->
      Lwt.return (durable, Inv_stop {stop with ticks = Z.pred stop.ticks})
          ...
          Lwt.catch
            (fun () ->
              ...
              | Host_func f ->
                  ...
                  ( durable,
                    Inv_stop {code = (vs', es); fresh_frame = None; ticks} )
              | Reveal_func f -> (
...
let label_step :
 ...
 fun ~init durable module_reg c buffers frame label_kont ->
  ...
  | LS_Craft_frame
      (Label_stack (label, stack), Inv_stop {code; fresh_frame; ticks})
    when ticks = Z.zero ->
