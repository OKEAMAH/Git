...
mod tests {
    ...
    pub fn queue_head<H: Runtime, E>(queue: &Queue, host: &H) -> Result<Option<E>, RuntimeError>
    where
    ...
    fn test_add_element() {
        ...
        let head: Element = queue_head(&queue, &mock_host).unwrap().unwrap();
    ...
    fn test_queue_correctly_saved() {
        ...
        let e1_fst: Option<Element> = queue_head(&first_queue, &mock_host).unwrap();
        let e2_fst: Option<Element> = queue_tail(&first_queue, &mock_host).unwrap();

        // get head and tail from the second queue
        let e1_snd: Option<Element> = queue_head(&second_queue, &mock_host).unwrap();
