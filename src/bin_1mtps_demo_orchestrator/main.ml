...
(* ---------------------------- *)
let submit_dac_messages ~round rollup_ =
...
let rec poll_for_level (client : Client.t) level =
  Log.info "----------------------" ;
  Log.info "Waiting for level %d" level ;
  Log.info "----------------------" ;
  let* got = Client.level client in
  if got >= level then return got
  else (
    Log.info "Got level %d" got ;
    poll_for_level client level)
...
let main () =
  ...
  let* node =
    if mode <> Nodes then (
      ...
      let* level = poll_for_level client (level + 1) in
      let r1 = Lwt_list.iter_p (submit_dac_messages ~round:1) nodes in
      let* level = poll_for_level client (level + 1) in
      let r2 = Lwt_list.iter_p (submit_dac_messages ~round:2) nodes in
      let* level = poll_for_level client (level + 1) in
      let r3 = Lwt_list.iter_p (submit_dac_messages ~round:3) nodes in
      let* level = poll_for_level client (level + 1) in
      let r4 = Lwt_list.iter_p (submit_dac_messages ~round:4) nodes in
      let* level = poll_for_level client (level + 1) in
      let r5 = Lwt_list.iter_p (submit_dac_messages ~round:5) nodes in
      let* level = poll_for_level client (level + 1) in
      let r6 = Lwt_list.iter_p (submit_dac_messages ~round:6) nodes in
      let* level = poll_for_level client (level + 1) in
      let r7 = Lwt_list.iter_p (submit_dac_messages ~round:7) nodes in
      let* _level = poll_for_level client (level + 1) in
      let r8 = Lwt_list.iter_p (submit_dac_messages ~round:8) nodes in
      let node, _, _, _, _ = List.hd nodes in
      let node = node.client.node in
      let+ _ = Lwt.join [r1; r2; r3; r4; r5; r6; r7; r8] in
      node)
    else
