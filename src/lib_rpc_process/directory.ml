...
let applied_blocks dir (store : Store.t option ref)
  ...
  gen_register0
    ...
    (fun q () ->
      ...
      let* store =
        ...
        | None -> Lwt.fail Not_found
      in
      Tezos_shell.Monitor_directory.applied_blocks
        store
        ~applied_blocks_watcher
        q
        ()) ;
...
let build_rpc_directory node_version config dynamic_store
  ...
  let static_dir =
    applied_blocks static_dir dynamic_store applied_blocks_watcher
  in
