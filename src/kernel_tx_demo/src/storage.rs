...
pub fn deposit_ticket_to_storage<Host: Runtime>(
    ...
    let mut account = account_storage.get_or_create_account(host, &path)?;
    ...
    tezos_smart_rollup_debug::debug_msg!(
        host,
        "Depositing {ticket:?} to {destination}\n"
    );
...
mod test {
    ...
    fn test_account_ticket_fund_limit() {
        ...
        // Assert
        a1.remove_ticket(&mut host, ticket_id, 11)
