...
use super::{
    Operation, OperationContent, OperationTransfer, OperationTransferCompressed,
    OperationWithdraw, TicketAmount, TicketIndex,
};
...
impl<'a> VerifiableOperation<'a> {
    ...
    pub fn execute<Host: Runtime>(
        ...
        if let Some(mut signer_account) =
            account_storage.get_account(host, &signer_path)?
        {
            ...
            match self.operation.contents {
                OperationContent::Withdraw(OperationWithdraw {
                    ...
                    debug_msg!(host, "withdrawing ticket");
                ...
                OperationContent::CTransfer(OperationTransferCompressed {
                    destination,
                    ticket: TicketIndex { amount: ticket },
                    amount: TicketAmount { amount },
                }) => {
                    handle_transfer(
                        host,
...
fn handle_transfer(
    ...
    let mut dest_account =
        account_storage.get_or_create_account(host, &dest_account_path)?;
    ...
    host.write_debug(unsafe {
        std::str::from_utf8_unchecked(&[id_0, id_1, colour, amount])
    });
