...
let get_wasm_pvm_state (type repo tree) (module Plugin : Protocol_plugin_sig.S)
  ...
  in
  let*! ctxt = Pvm.Context.checkout context context_hash in
  let* ctxt =
    match ctxt with
    | None ->
        tzfail
...
let set_value_instr (type repo tree) ~(pvm : (repo, tree) Pvm_plugin_sig.plugin)
    key tree =
  let open Lwt_syntax in
