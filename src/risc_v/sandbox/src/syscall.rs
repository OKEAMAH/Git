...
use crate::rv::{A0, A1, A2, A3, A4, A5, A6, A7};
...
use tezos_smart_rollup_constants::riscv::{
    ...
    SBI_TEZOS_ED25519_VERIFY, SBI_TEZOS_INBOX_NEXT, SBI_TEZOS_META_ADDRESS,
...
fn sbi_tezos_ed25519_verify(emu: &mut Emulator) -> SBIResult {
    ...
    let pk_len = emu.cpu.xregs.read(A1);
    let pk_bytes = read_memory(emu, pk_addr, pk_len)?;

    let sig_addr = read_physical_address(emu, A2)?;
    let sig_len = emu.cpu.xregs.read(A3);
    let sig_bytes = read_memory(emu, sig_addr, sig_len)?;

    let msg_addr = read_physical_address(emu, A4)?;
    let msg_len = emu.cpu.xregs.read(A5);
