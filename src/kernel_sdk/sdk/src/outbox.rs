...
//! ## Example usage
//!
//! Say we wanted to send a message to the outbox, for every inbox message receieved. There may be more
...
use tezos_smart_rollup_core::{MAX_FILE_CHUNK_SIZE, MAX_OUTPUT_SIZE};
...
impl<'a, P: Path> OutboxQueue<'a, P> {
    ...
    pub fn new(root: &'a P, max: u32) -> Result<Self, PathError> {
        ...
        let meta = concat(root, &META_SUFFIX).unwrap();
    ...
    pub fn flush_queue(&self, host: &mut impl Runtime) -> usize {
        let mut buffer = [0; MAX_OUTPUT_SIZE];
        ...
        loop {
            ...
            // store_read_slice should handle this loop
            let mut read = host
                .store_read_slice(&next, 0, &mut buffer[..MAX_FILE_CHUNK_SIZE])
                .unwrap();
            if read == MAX_FILE_CHUNK_SIZE {
                read += host
                    .store_read_slice(
                        &next,
                        MAX_FILE_CHUNK_SIZE,
                        &mut buffer[MAX_FILE_CHUNK_SIZE..],
                    )
                    .unwrap();
            };

            if host.write_output(&buffer[..read]).is_err() {
                // Outbox is full
                return num_flushed;
            };
    ...
    fn save_meta(&self, host: &mut impl Runtime, start: u32, len: u32) {
        if len == 0 {
            host.store_delete(self.root).unwrap();
            return;
        }
